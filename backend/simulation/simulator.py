...
class BitTorrentSimulator:
    ...
    Each peer is a SimPy process. A peer independently waits, chooses a missing
    chunk, reserves upload/download slots, waits for virtual transfer time, and
    immediately updates its state when the transfer event finishes.
    """
    ...
    def _transfer(self, destination: Peer, source: Peer, chunk_id: int) -> Generator[Any, Any, None]:
        ...
        duration, bandwidth, latency_ms = self.network.transfer_time(
            self.config.chunk_size_kb,
            source,
            destination,
        )
    ...
    def _peer_process(self, peer: Peer) -> Generator[Any, Any, None]:
        # Stagger process startup so ties are deterministic but not perfectly synchronized.
        yield self.env.timeout(self.rng.uniform(0, self.config.polling_interval))

        while not self.completed and not peer.is_complete(self.config.total_chunks):
            ...
            yield from self._transfer(peer, source, chunk_id)
